...
- **Cero lint.** Typecheck y tests sí hay: `yarn typecheck` desde `ff233c5`, y `yarn test` desde el commit que agregó `test/seo.test.mjs`. Son dos archivos: `test/seo.test.mjs` cubre el contrato de metadata (presencia, unicidad, longitud, separador de breadcrumb) sobre las cinco páginas, y `test/og-cards.test.mjs` cubre las share cards (que la card del home diga lo mismo que el H1, que el manifest commiteado corresponda a la definición vigente, y que los PNG midan 1200x630). Ojo con el alcance del segundo: compara un digest del SVG de origen, **no los píxeles**, así que atrapa la edición de copy que se olvidó de regenerar, pero no un PNG reemplazado a mano. Ninguno de los dos rasteriza: `sharp` no es dependencia declarada del proyecto — llega como opcional de astro — y librsvg resuelve fuentes del sistema, así que el mismo input da píxeles distintos en Linux y en macOS. No hay ninguna prueba de comportamiento del sitio: nada verifica el drawer, el toggle de tema, el filtro del portafolio ni las animaciones de scroll.
